...
The Filters button at the top of the page offers options to look at more granular views of your trip data. The filters set with this button apply to the statistics cards and graphs as well as the data in the By Destination table.
...
- **`Repeat customers`**: the total number of users who have completed more than one trip. Like the other statistic cards, the number of repeat customers is affected by the filters set in the Filters section.

The graphs for each card show the change over time in each statistic. The time windows of each graph are automatically determined by the date range set in the Filters button. To see data graphed by hour, set the date range to encompass 2 days or fewer. To see data graphed by day, set the date range to encompass 21 days or fewer. To see data graphed by weeks, set the date range to encompass 60 days or fewer. To see data graphed by month, the date range must encompass more than 60 days of data.
...
- **`Geofence ID`**: the [external ID](https://radar.io/documentation/geofences#csv-import) of the unique geofence destination listed in the destination column.
...
**By Destination table filter box:** The text box in the By Destination table on the Analytics page also allows you to filter the data in the table. You can filter by Destination, Tag, or Geofence ID. (The filters set from the text box only apply to data in the By Destination table and do not apply to the overall statistic cards at the top of the page.)

**Additional options in the table:** On the right side of each row, there is an icon with three dots that allows you to select additional options. Clicking on this icon offers two actions:

- **`Filter by destination`**: This will filter the entire analytics page to only include this single destination. This is the equivalent of using the Filter button at the top of the page.
